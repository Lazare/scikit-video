...
def test_FFmpegReader():
    ...
    C = 0
    accumulation = 0
    with skvideo.io.FFmpegReader(skvideo.datasets.bigbuckbunny(), verbosity=0) as reader:
...
def test_FFmpegReader_fps():
    ...
    with skvideo.io.FFmpegReader(skvideo.datasets.bigbuckbunny(), outputdict={"-r": "10"}, verbosity=0) as reader:
        for frame in reader.nextFrame():
            M, N, C = frame.shape
            accumulation += np.sum(frame)
            T += 1
...
def test_FFmpegReader_16bits():
    ...
    with skvideo.io.FFmpegReader(skvideo.datasets.bigbuckbunny(), outputdict={'-pix_fmt':'rgb48le',"-r": "5"}, verbosity=0) as reader16, \
         skvideo.io.FFmpegReader(skvideo.datasets.bigbuckbunny(), outputdict={'-pix_fmt': 'rgb24', "-r": "5"},verbosity=0) as reader8:
